...
public class PatientMedicalRecordController {
    ...
    @GetMapping("/my")
    public ResultVo<List<MedicalRecordVo>> myRecords() {
        Long patientId = getCurrentPatientId();
        List<MedicalRecordVo> list = medicalRecordService.getPatientHistory(patientId);
        list = list.stream()
    ...
    @GetMapping("/{recordId}")
    public ResultVo<MedicalRecordVo> detail(@PathVariable Long recordId) {
        Long patientId = getCurrentPatientId();
        return ResultVo.ok(medicalRecordService.getRecordDetailForPatient(recordId, patientId));
    }
